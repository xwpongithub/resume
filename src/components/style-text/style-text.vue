...
<script>
  ...
  import { handleChar, getLanguage } from '../../common/js/util'
  const styleText = [0, 1, 2].map((i) => {
    if (getLanguage().indexOf('ja') > -1) return require('!raw-loader!./styles' + i + '-ja.css')
    else if (getLanguage().indexOf('zh') > -1) return require('!raw-loader!./styles' + i + '.css')
  })
